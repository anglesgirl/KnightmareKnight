...
var pluginName = 'TRP_Particle_Editor';
...
(function(){
	ParticleEditor.GUIDE_TEXTS = {
		...
		burst:['【バースト(発生方法)】','particlesPerWaveで規則的に発生させる','ps:射出角度の差(particleSpacing)','sa:開始角度。ps*sa=360とすると放射状に射出(startAngle)','dt:射出時間の差(deltaTime)','rt:0=開始角度(startRotation)無視','└1=開始角度優先,2=開始角度を加算(rotationType)','r:半径(radius)','dr:半径の差。マイナスで1周ごとの差(deltaRadius)','dx:xの差(deltaX)','dy:yの差(deltaY)','<※発生方法はいずれか１つのみ指定が有効>'],

		sub:['【サブエミッター設定】','st:射出開始タイミング(0~1)','et:射出終了タイミング(0~1。-1で無効)','spd:慣性の影響度合い。0=影響なし、1=100%影響。','iAngle:1=開始角度は慣性方向基準','iScale:1=親パーティクルの拡大率乗数を継承']
	};
...
Game_Particle.prototype._particleEdit = function(eventId,copyName,id,target,name,z,x,y,image,exData=null){
	...
	if(!data){
		if(!subId){
			...
			throw new Error('管理ID「'+mainId+'」は再生されてません。')
		}
	}
	...
	for(var i=0; i<2; i=(i+1)|0){
	    ...
	    for(var j=0; j<categoryLen; j=(j+1)|0){
	        ...
	        if(isSetCommand){
				...
				texts += '\n * @text set/表示 > '+categoryText+'('+keysLen+')';
				texts += '\n * @desc パーティクル表示';
	        }else{
	        	texts += '\n * @command play_'+category;
				texts += '\n * @text play/１回再生 > '+categoryText+'('+keysLen+')';
				texts += '\n * @desc パーティクルを１回だけ再生';
	        }
    		texts += '\n *';
			texts += '\n * @arg id';
...
PresetPicker.CATEGORY_NAMES = ['キャラ対象','スクリーン/天候/リージョン','マウス/タップ','その他','非表示(/h)'];
